...
func collect(urls []int) []string {
	...
	// 关闭接收结果channel
	//这里必须要先关闭 responseChannel 通道，然后再执行wgResponse.Wait()，不然会fatal error: all goroutines are asleep - deadlock!，为什么？？
	//因为上面在读取 responseChannel 的协程还在执行，一直在等待向 responseChannel 中写入数据，一直没有执行 wgResponse.Done()
